...
# Create a temp dataset with the number of instances of each State+District. Ideally, all combos should have 6 instances
# indicating that they are present in all years
dist_counts <- df %>% count(State, District) %>% arrange(desc(n))
unmatched_dists <- dist_counts %>% filter(n <6) %>% arrange(State, District)


# There are about 340 districts for which we have all 6 years of data without further cleaning.
# This is good enough for me.
full_dists <- dist_counts %>% filter(n ==6) %>% select(State, District)
df_partial <- df %>% inner_join(full_dists, by = c("State", "District")) %>% 
  select(State, District, year, std12_letters_and_up, std12_numbers_and_up, std35_std1_and_up, std35_subtraction_and_up)

write_csv(df_partial, "C:/Users/dougj/Documents/Data/Education/ASER District Data/Clean/aser_district_partial.csv")
