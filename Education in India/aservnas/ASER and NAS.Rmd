...
main_data <- read_csv("ASER 2018 and NAS 2017 grade 3 reading.csv")
...
## Plot ASER and NAS on same scale
...
```{r}
df.long <- df %>% select(State, ASER, NAS) %>%
  gather(assessment, avg_score, -State)
ggplot(data = df.long, aes(x= State,y=avg_score, fill= assessment)) + 
