...
Our final value of $\sigma_{s}^2$ for districts is .0011.  For states, we calculate $\sigma_{s}^2=.0011/(ND_s)$ where $ND_s$ is the number of districts in state s and then take the average across states. We note that the proportion of variance in state levels and changes due to sampling is very small and thus estimating state level sampling variance accurately is less important than for districts.
...
We focus mainly on the third learning outcome -- the share of class 3-5 students who can read a level 1 text.  This is because this outcome is similar to the outcome used to compare ASER to NAS.
...
output <- "C:/Users/dougj/Dropbox/Education in India/Original research/Learning outcomes data/figures/aser_reading_reliability"
...
subject <- "math"
if (subject == "math") {
  dists$std12_letters_and_up <- dists$std12_numbers_and_up
  states$std3_reading <- states$std3_math
  output <- "C:/Users/dougj/Dropbox/Education in India/Original research/Learning outcomes data/figures/aser_math_reliability"
}
...
var_dist_deltas <- var(dist_deltas$delta_letters/100, na.rm = TRUE)
...
var_dist_levels <- var(dists$std12_letters_and_up/100, na.rm = TRUE)
...
corr_dist_dbl_lag <- dist_deltas %>% filter(year >= 2009) %>%
  ungroup() %>%
  summarize(auto =cor(delta_letters,delta_letters_dbl_lag, use = "pairwise.complete.obs")) %>%
  .$auto
...
* Share of the transitory variance due to sampling (for both changes and levels) = `r .0011/var_dist_epsilon`
...
ggsave("correlation_decay.png", width = 5, height = 6 , path = output)
```
...
df_bar$value[(df_bar$state_or_dist == "District") & (df_bar$changes_or_levels == "Changes") & (df_bar$bar_part == "Transitory sampling")] <- 2*.0011

df_bar$value[(df_bar$state_or_dist == "District") & (df_bar$changes_or_levels == "Changes") & (df_bar$bar_part == "Transitory other")] <- 2*var_dist_epsilon - 2*.0011
...
df_bar$value[(df_bar$state_or_dist == "District") & (df_bar$changes_or_levels == "Levels") & (df_bar$bar_part == "Transitory sampling")] <- .0011

df_bar$value[(df_bar$state_or_dist == "District") & (df_bar$changes_or_levels == "Levels") & (df_bar$bar_part == "Transitory other")] <- var_dist_epsilon - .0011
...
ggsave("variance_decomposition.png", width = 5, height = 6 , path = output)
...
write_csv(df_final, file.path(output, "variance_decomposition.csv"))
```
